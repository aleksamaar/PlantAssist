...
`sortedBySection()` сортирует по **сроку следующего действия** (`sortKeyFor`), а не
по дате последнего: частоты у растений разные, и «полили 25 дней назад» с циклом
60 не срочнее, чем «20 дней назад» с циклом 14.

Приоритеты выше срока: галочка `needs_repotting` поднимает растение в начало
списка пересадки, заглушённые напоминания уходят в конец. Пустой ключ означает
«срока нет вовсе» → в конец; `'0000-01-01'` — «ждёт с самого начала» (расписание
задано, отметок нет) → в начало. Для пересадки без единой отметки срок считается
от даты покупки + 365.
...
## Иллюстрации достижений

У достижения могут быть поля `img` (иконка в карточке, 240px) и `imgLarge`
(просмотр по клику, 760px) — оба WebP в `static/icons/achievements/`. Если файла
нет, молча показывается эмодзи из `icon`, поэтому картинки добавляются по одной.

Пересборка: `py tools/make_achievement_icons.py`. Скрипт берёт квадратные JPG из
`design/achievement-sources/` (папка в `.gitignore`, готовые иконки коммитятся),
обрезает по кругу и убирает белые углы — иначе они торчат на плитке карточки.
Добавить новую: положить файл, дописать строку в `JOBS`, запустить, прописать
`img`/`imgLarge` у достижения в `app.js`.

**Картинки грузятся отложенно** (`observeAchievementIcons`): `<img>` создаётся,
только когда карточка подошла к экрану, до этого в плитке эмодзи. Достижений 71,
и грузить все картинки сразу — это мегабайты ради невидимого.
